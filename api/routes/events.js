...
router
  ...
  .post('/', (req, res) => {
    ...
    req.body.slug = req.body.title_en ? slug(req.body.title_en) : slug(req.body.title_vi)
  ...
  .post( '/upload', upload.single( 'file' ), function( req, res, next ) {
    ...
    if (req.type == 'poster') {
      if ( ( dimensions.width != 350 ) || ( dimensions.height != 495 ) ) {
        return res.status( 422 ).json( {
          error : 'Poster must be 350x495 px'
        } );
      }
    }
